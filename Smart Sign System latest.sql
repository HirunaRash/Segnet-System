...
-- Transactions table (one record per cheque approval request)
...
-- TransactionSignatures table (one record per employee who signs)
CREATE TABLE TransactionSignatures (
    TransactionSignatureId  INT AUTO_INCREMENT PRIMARY KEY,
    TransactionId            INT NOT NULL,
    EmployeeId               INT NOT NULL,           -- employee who signed
    SignatureId              INT NOT NULL,            -- which signature image was used
    ...
    FOREIGN KEY (EmployeeId)    REFERENCES Employees(EmployeeId),
    FOREIGN KEY (SignatureId)   REFERENCES EmployeeSignatures(SignatureId)
);
